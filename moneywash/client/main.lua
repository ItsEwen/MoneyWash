	...
	while true do
		...
		if dist > 50 then
			if inBlanchiment then
				...
				inBlanchiment = false
				ESX.ShowNotification('~r~YourNameServer ~w~~n~Vous vous êtes éloigner de la zone')
				Counter = true
			end
		end
		if dist < 50 then
			...
			if IsControlJustPressed(1,51) then
				if not inBlanchiment then
					if not Counter then
						...
						ESX.ShowNotification('~r~YourNameServer ~y~~n~ANTI GLITCH~n~~w~Vous allez trop vite pour le système')
					end
				else
					TriggerServerEvent('::{korioz#0110}::esx_moneywash:stopWash')
					inBlanchiment = false
					ESX.ShowNotification('~r~YourNameServer ~w~~n~Vous avez arrêter de blanchir')
					Counter = true
				end
			end
		end
